<script lang="ts">
  ...
  function resetView() {
    const src = map.getSource("climate-tiles") as
  ...
  onMount(() => {
    ...
    map.on("mousemove", (e) => {
      ...
      const bounds = (
        map.getSource("climate-tiles") as
          maplibregl.RasterTileSource | undefined
      )?.bounds;
      if (bounds) {
...
<div class="map-wrapper">
  ...
  <button
    ...
    onclick={resetView}
    aria-label="Reset view">&#8635; Reset View</button
  >
  {#if hoverValue !== null}
